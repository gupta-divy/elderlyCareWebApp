...
import { TASK_GRACE_PERIOD_MINUTES, buildOccurrenceId, getNextOccurrenceAfterTask, getNextOccurrenceForInput, isOccurrenceOverdue, } from '../features/tasks/taskRecurrence';
...
export function getOccurrenceForTask(occurrences, taskId, scheduledFor) {
...
export function getTasksForParent(state, parentId) {
    return state.taskTemplates
        .filter((task) => task.assignedParentId === parentId && isTaskVisible(task))
        .map((task) => {
        const occurrence = getPendingOccurrenceForTask(state.taskOccurrences, task.id) ??
            state.taskOccurrences
...
export function getTaskSummaryForParent(state, parentId) {
...
export function saveTaskDefinition(state, input, actor, taskId, now = new Date()) {
...
export function completeTaskOccurrence(state, occurrenceId, completedBy, options, now = new Date()) {
...
export function reopenTaskOccurrence(state, occurrenceId, now = new Date()) {
    const occurrence = state.taskOccurrences.find((entry) => entry.id === occurrenceId);
    if (!occurrence)
        return state;
    const nextState = {
...
export function deleteTaskDefinition(state, taskId, now = new Date()) {
    const task = state.taskTemplates.find((entry) => entry.id === taskId);
    if (!task)
        return state;
    const nextTask = {
...
export function setTaskActive(state, taskId, isActive, now = new Date()) {
    const task = state.taskTemplates.find((entry) => entry.id === taskId);
    if (!task)
        return state;
    const nextTask = {
...
export function markMissedTaskOccurrences(state, now = new Date()) {
    let nextState = state;
    for (const occurrence of state.taskOccurrences) {
        if (occurrence.status !== 'pending')
            continue;
        if (!isOccurrenceOverdue(occurrence, now, TASK_GRACE_PERIOD_MINUTES))
            continue;
...
export function normalizeTaskState(state, now = new Date()) {
