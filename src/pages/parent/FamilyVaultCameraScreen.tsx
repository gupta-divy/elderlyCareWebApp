...
import { addFamilyVaultPhoto } from '../../features/photos/familyVaultStore';
...
export function FamilyVaultCameraScreen() {
  ...
  const handleShareToVault = () => {
    if (!capturedPhoto || !selectedParent || isSaving) return;

    setIsSaving(true);
    addFamilyVaultPhoto(capturedPhoto);
        ...
        {!capturedPhoto ? (
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <button
              ...
              disabled={isSaving}
              className="min-h-[84px] rounded-[28px] bg-teal-400 px-4 text-2xl font-bold text-slate-950 shadow-[0_14px_32px_rgba(45,212,191,0.35)] disabled:opacity-60"
            >
              {isSaving ? 'Sharing...' : 'Share to Vault'}
            </button>
          </div>
        )}
      </div>
